...
function bind_clib()
	...
	function funcs.SetWindowSizeLimits(
		window,
		minwidth,
		minheight,
		maxwidth,
	...
	function funcs.SetWindowMonitor(
		window,
		monitor,
		xpos,
		ypos,
	...
	function funcs.GetJoystickButtons(jid, buttons)
		local count = ffi.new('int[1]')
		local cbuttons =
			clib.glfwGetJoystickButtons(aux.get_const(const, jid), count)
	...
	function funcs.GetJoystickName(jid)
		return aux.string_or_nil(
			const,
			clib.glfwGetJoystickName(aux.get_const(const, jid))
		)
	end
	...
	function funcs.GetJoystickGUID(jid)
		return aux.string_or_nil(
			const,
			clib.glfwGetJoystickGUID(aux.get_const(const, jid))
		)
	end
	...
	function funcs.GetGamepadName(jid)
		return aux.string_or_nil(
			const,
			clib.glfwGetGamepadName(aux.get_const(const, jid))
		)
	end
	...
	if args.bind_vulkan == true then
		...
		function funcs.GetPhysicalDevicePresentationSupport(
			instance,
			device,
			queuefamily
		)
			return clib.glfwGetPhysicalDevicePresentationSupport(
				instance,
				device,
				queuefamily
			)
